...
function clearDemoData() {
    ...
    console.log('🗑️ All mock data cleared - using real API data only');
}
...
class Dashboard {
    ...
    verifyLicense(contractorId) {
        ...
        try {
            ...
            if (!verificationWindow) {
                ...
                this.showNotification('⚠️ Please allow popups and try again', 'warning');
    ...
    removePendingContractor(contractorId) {
        console.log('🗑️ Removing contractor from pending list:', contractorId);
        ...
        users.forEach(user => {
            ...
            userItem.innerHTML = `
                ...
                <div class="user-actions">
                    ...
                    <button class="btn-small danger" onclick="window.dashboard.deleteUser(${user.id})">🗑️ Delete</button>
                </div>
            `;
    ...
    async deleteUser(userId) {
        console.log('🗑️ Delete user called with ID:', userId);
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="delete-consequences">
                        <h5>⚠️ Consequences of deletion:</h5>
                        <ul>
    ...
    async confirmUserDeletion(userId) {
        ...
        if (userToDelete) {
            try {
                ...
                this.showNotification(`🗑️ User "${userToDelete.name}" deleted successfully`, 'warning');
    ...
    clearAllData() {
        if (confirm('⚠️ DANGER: This will delete ALL data including referrals, users, and settings. This cannot be undone. Are you absolutely sure?')) {
            if (confirm('🚨 FINAL WARNING: All data will be permanently deleted. Type "DELETE" to confirm.')) {
                localStorage.clear();
                
                // Reload page to reset everything
                this.showNotification('🗑️ All data cleared. Reloading...', 'warning');
                setTimeout(() => {
                ...
                <div class="modal-body">
                    ...
                    <div class="system-info">
                        ...
                        <ul style="list-style: none; padding: 0;">
                            ...
                            <li>🚧 Maintenance Mode: ${settings.system.maintenanceMode ? '⚠️ ACTIVE' : '✅ Normal'}</li>
                        </ul>
                    </div>
                </div>
                <div class="modal-footer">
...
window.suspendContractor = function(contractorId) {
    if (confirm('⚠️ Are you sure you want to suspend this contractor?\n\nThis will prevent them from receiving new referrals.')) {
        ...
        window.dashboard.showNotification(`⚠️ Contractor ${contractorId} has been suspended`, 'warning');
...
window.clearAllReferrals = function() {
    if (confirm('⚠️ This will delete all referrals. Are you sure?')) {
        localStorage.removeItem('referrals');
        window.dashboard.loadReferrals();
        window.dashboard.updateReferralStats();
        window.dashboard.showNotification('🗑️ All referrals cleared', 'warning');
    }
};
...
window.saveSystemSettings = function() {
    if (window.dashboard) {
        ...
        if (settings.system.maintenanceMode) {
            setTimeout(() => {
                window.dashboard.showNotification('⚠️ Maintenance mode is now ACTIVE - system access may be limited', 'warning');
            }, 1000);
        }
    }
};
...
window.clearAllContractorData = function() {
    ...
    localStorage.removeItem('contractors');
    localStorage.removeItem('rejectedContractors');
    console.log('🗑️ All contractor data cleared');
